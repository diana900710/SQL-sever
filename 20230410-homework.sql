...
select top 3 
	SUM(od.UnitPrice*od.Quantity*(1-od.Discount)) as Sum_Price, c.CustomerID
from Customers c
...
from Products p
left join [Order Details] od on p.ProductID = od.ProductID
left join Orders o on od.OrderID = od.OrderID
group by p.CategoryID, p.ProductID
order by SUM(od.UnitPrice*od.Quantity*(1-od.Discount)) desc
)
select top 3 
	SUM(od.UnitPrice*od.Quantity*(1-od.Discount)) as Sum_Catagory, c.CustomerID
from Customers c
inner join Orders o on c.CustomerID = o.CustomerID
inner join [Order Details] od on o.OrderID = od.OrderID
inner join Products p on od.ProductID = p.ProductID
inner join Categories ca on p.CategoryID = ca.CategoryID
inner join cat on ca.CategoryID = cat.CategoryID
...
-- 列出最少人買的產品
select top 1
	p.ProductID, COUNT(od.Quantity) as ProductAmount
from Products p
left outer join [Order Details] od on p.ProductID = od.ProductID
group by p.ProductID
...
WITH o0 AS (
    SELECT s.ShipperID, c.CategoryID, COUNT(*) AS CategoryAmount
    FROM Shippers s
    LEFT OUTER JOIN Orders o ON s.ShipperID = o.ShipVia
    LEFT OUTER JOIN [Order Details] od ON o.OrderID = od.OrderID
    LEFT OUTER JOIN Products p ON od.ProductID = p.ProductID
    LEFT OUTER JOIN Categories c ON p.CategoryID = c.CategoryID
    GROUP BY s.ShipperID, c.CategoryID
), o1 AS (
    SELECT ShipperID, CategoryID, CategoryAmount,
        ROW_NUMBER() OVER(
...
select
	*
from p1
where p1.PriceRank =1
-- 列出每一個客戶買最多的那一個產品與購買數量
with amo1 as(
select
	c.CustomerID, p.ProductID, SUM(od.Quantity) ProductAmount
from Customers c
left outer join Orders o on c.CustomerID=o.CustomerID
left outer join [Order Details] od on o.OrderID=od.OrderID
left outer join Products p on od.ProductID=p.ProductID
...
as
select distinct ShipCity, MAX(ShippedDate) 
from Orders
